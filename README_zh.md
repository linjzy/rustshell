...
共用同一条底层会话。同一设备、同一通道的调用必须顺序执行，不能并发
提交；空闲 300 秒后会话自动关闭，下一次调用只重连一次；会话启动连续失败时，MCP 会对同一设备和通道设置 10 秒冷却，避免并发或快速重试把失败放大；
...
大文件传输没有服务端总时长限制；首次取得有效传输进度前连续 45 秒没有协议进度
就停止，已经开始传输后连续 300 秒没有协议进度才停止。这样能快速识别文件通道
未建立或零字节无响应的设备，同时保留大文件和断点续传的容错窗口。RustShell
自身版本与握手上报的 RustDesk 1.4.9 协议版本彼此独立，以便
